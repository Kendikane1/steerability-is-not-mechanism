"""P1-014: six explicitly scoped offline synthetic repeat/capture/identity passes.

Launch in a fresh process with HF_HUB_OFFLINE=1 TRANSFORMERS_OFFLINE=1 and an unused run directory.
Only capture and identical-copy replacement; no generation, scientific data or fallback.
"""
...
from steerability_is_not_mechanism.noop_check import run_noop_check

ROOT = Path(__file__).resolve().parents[1]
parser = argparse.ArgumentParser(description=__doc__)
parser.add_argument("--request", type=Path, default=ROOT / "configs/local_noop.yaml")
parser.add_argument("--run-dir", type=Path, required=True)
args = parser.parse_args()
protocol_path = ROOT / "configs/local_model_engineering.yaml"
request, spec = read_noop_request(args.request, protocol_path)
...
try:
    ...
    adapter, runtime = load_noop_adapter(
        request,
        spec,
    ...
    run_noop_check(adapter, ids, spec.checks, on_start, on_result)
    if device.type == "mps":
        record["mps_after"] = {
    ...
    print("All six no-op checks passed", flush=True)
