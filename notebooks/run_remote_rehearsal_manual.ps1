param([switch]$CheckOnly, [ValidateSet('manual-rehearsal-02','manual-rehearsal-03')][string]$Attempt='manual-rehearsal-02')
$ErrorActionPreference='Stop'
$project=Split-Path $PSScriptRoot -Parent
Set-Location $project
$manifest='outputs/phase1/p1-023/manual-source-hashes-03.json'
...
$output="outputs/phase1/p1-023/$Attempt"
...
try {
    ...
    & '.\.venv\Scripts\python.exe' -B notebooks/check_remote_rehearsal.py --output-dir $output
    $code=$LASTEXITCODE
    ...
    if(Test-Path $report) {
        ...
        [pscustomobject]@{
            ...
            long_storage_feasible=$r.long_storage_feasible
            estimated_long_output_GiB=if($null -ne $r.estimated_long_output_bytes) {[math]::Round($r.estimated_long_output_bytes/1GB,3)} else {$null}
            report=$report
