"""P1-023 CUDA worker: 12 fixed cycles, sequential shards, one model per process."""
...
from steerability_is_not_mechanism.remote_cuda import CudaMonitor, configure_cuda
from steerability_is_not_mechanism.remote_rehearsal import load_adapter, read_request, source_hashes
...
try:
    request, spec = read_request(
        ROOT / "configs/remote_rehearsal.yaml", ROOT / "configs/local_model_engineering.yaml"
    )
    ...
    for shard in range(3):
        ...
        store = (CompactShardStore if args.compact else ShardStore)(
        ...
        for job in jobs:
            ...
            def compute(job=job):
                ...
                if adapter is None:
                    ...
                    save()
                    monitor = CudaMonitor(args.attempt_dir, ROOT, seconds=600)
